...
class ScriptsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create script" do
    assert_difference('Script.count') do
      post scripts_url, params: { script: { image: @script.image, import: @script.import, link: @script.link, name: @script.name } }
    end
  ...
  test "should update script" do
    patch script_url(@script), params: { script: { image: @script.image, import: @script.import, link: @script.link, name: @script.name } }
